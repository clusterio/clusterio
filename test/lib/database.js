...
describe("lib/database", function() {
	describe("mapToObject()", function() {
		it("should throw on non-string key", function() {
			assert.throws(
				() => libDatabase.mapToObject(new Map([[1, 1]])),
				new Error("Expected all keys to be string but got number")
			);
			assert.throws(
				() => libDatabase.mapToObject(new Map([[Symbol("test"), 1]])),
		...
		it("should throw convert a map to an object", function() {
			assert.deepEqual(
				libDatabase.mapToObject(new Map([["a", 1], ["b", true]])),
				{ "a": 1, "b": true }
			);
		});
	});
	...
	describe("loadJsonAsMap()", function() {
		...
		for (let type of badTypes) {
			it(`should reject on ${type} JSON`, async function() {
				await assert.rejects(
					libDatabase.loadJsonAsMap(path.join("test", "file", "json", `${type}.json`)),
		...
		it("should work on empty object JSON", async function() {
			assert.deepEqual(
				await libDatabase.loadJsonAsMap(path.join("test", "file", "json", "object.json")),
				new Map()
			);
		});
		...
		it("should work on object JSON", async function() {
			assert.deepEqual(
				await libDatabase.loadJsonAsMap(path.join("test", "file", "json", "load_map.json")),
				new Map([["a", 1], ["b", true]])
			);
		});
		...
		it("should give an empty Map for non-existant file", async function() {
			assert.deepEqual(
				await libDatabase.loadJsonAsMap(path.join("test", "file", "json", "does-not-exist")),
				new Map()
			);
		});
	});
	...
	describe("saveMapAsJson()", function() {
		it("should save a mapping as JSON", async function() {
			...
			await libDatabase.saveMapAsJson(
	...
	describe("loadJsonArrayAsMap()", function() {
		...
		for (let type of badTypes) {
			it(`should reject on ${type} JSON`, async function() {
				await assert.rejects(
					libDatabase.loadJsonArrayAsMap(path.join("test", "file", "json", `${type}.json`)),
		...
		it("should reject on null element", async function() {
			await assert.rejects(
				libDatabase.loadJsonArrayAsMap(path.join("test", "file", "json", "array_null.json")),
		...
		it("should reject on element missing id", async function() {
			await assert.rejects(
				libDatabase.loadJsonArrayAsMap(path.join("test", "file", "json", "array_object.json")),
		...
		it("should work on empty array JSON", async function() {
			assert.deepEqual(
				await libDatabase.loadJsonArrayAsMap(path.join("test", "file", "json", "array.json")),
				new Map()
			);
		});
		...
		it("should work on array JSON", async function() {
			assert.deepEqual(
				await libDatabase.loadJsonArrayAsMap(path.join("test", "file", "json", "load_array_map.json")),
		...
		it("should give an empty Map for non-existant file", async function() {
			assert.deepEqual(
				await libDatabase.loadJsonArrayAsMap(path.join("test", "file", "json", "does-not-exist")),
				new Map()
			);
		});
	});
	...
	describe("saveMapAsJsonArray()", function() {
		it("should save a mapping as JSON", async function() {
			...
			await deleteTestFile();
			await libDatabase.saveMapAsJsonArray(
	...
	describe("class ItemDatabase", function() {
		...
		describe(".serialize()", function() {
			it("should return a serialized database", function() {
				let items = new libDatabase.ItemDatabase({ "a": 10 });
				assert.deepEqual(items.serialize(), { "a": 10 });
			});

			it("should remove zero count entries", function() {
				let items = new libDatabase.ItemDatabase({ "a": 0 });
		...
		describe(".removeItem()", function() {
			it("should remove an existing item", function() {
				let items = new libDatabase.ItemDatabase({ "a": 20 });
