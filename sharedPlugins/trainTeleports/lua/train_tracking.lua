...
script.on_event(defines.events.on_train_changed_state, function (event)
	...
	elseif oldTrainState == defines.train_state.wait_station then
		log(":)")
		for i,v in ipairs(global.trainsToSend) do
			if v and v.train == entity then
				game.print("No longer ready to send train!")
				table.remove(global.trainsToSend, i)
				entity.locomotives.front_movers[1].color = {r=1,g=0,b=1}
			end
		end
	end
end)
...
local function find_train_at_station(stationName)
    for _, train in pairs(game.forces.player.get_trains(game.surfaces[1])) do
        if train.station ~= nil and train.station.backer_name == stationName then
            return train
        end
    end
end
