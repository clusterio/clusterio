...
/c game.write_file("test.txt",serpent.block(game.players["Danielv123"].get_inventory(defines.inventory.character_main).get_contents()))
...
for a,b in pairs(game.players) do
	if b.connected and b.character then
		...
		local inventory = game.players[a].get_inventory(defines.inventory.character_main).get_contents()
