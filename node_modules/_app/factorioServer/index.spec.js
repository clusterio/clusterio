...
describe("factorioServer/index.js", ()=>{
	...
	describe(".createMap()", ()=>{
		it("creates a map file as instanceDirectory/saves/map.zip", async ()=>{
			let relativePath = __dirname.replace(appRootPath, "").substr(1);
			let server = new factorioServer({
				instanceDirectory: path.resolve(appRootPath+"", relativePath, "_testData_createMap"),
				instanceName: "Unit testing instance",
			});
			await server.initialize();
			console.log(appRootPath+"")
			console.log(relativePath)
			console.log(path.resolve(appRootPath+"", relativePath, "_testData_createMap", "saves"))
			let maps = fs.readdirSync(path.resolve(appRootPath+"", relativePath, "_testData_createMap", "saves"));
			assert.equal(maps.length, 0);
			await server.createMap();
			let maps2 = fs.readdirSync(path.resolve(appRootPath+"", relativePath, "_testData_createMap", "saves"));
