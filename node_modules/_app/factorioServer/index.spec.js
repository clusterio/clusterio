...
describe("factorioServer/index.js", ()=>{
	...
	describe(".createMap()", ()=>{
		it("creates a map file as instanceDirectory/saves/map.zip", async ()=>{
			...
			let server = new factorioServer({
				instanceDirectory: path.resolve(relativePath, "_testData_createMap"),
				instanceName: "Unit testing instance",
			});
			await server.initialize();
			console.log("Relative path: "+relativePath)
			let maps = fs.readdirSync(path.resolve(relativePath, "_testData_createMap", "saves"));
			assert.equal(maps.length, 0);
			await server.createMap();
			let maps2 = fs.readdirSync(path.resolve(relativePath, "_testData_createMap", "saves"));
