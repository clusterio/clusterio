...
module.exports = class factorioServer {
	...
	async createMap(name = "map.zip"){
		...
		let code = child_process.spawnSync(
			path.resolve(this.factorioDirectory, 'bin', 'x64', 'factorio'), [
				'-c', path.resolve(this.instanceDirectory,'config.ini'),
				'--create', path.resolve(this.instanceDirectory, 'saves',name),
				'--server-settings', path.resolve(this.instanceDirectory, 'server-settings.json'),
			],{
				cwd:process.cwd(),
			}
		);
		console.log(code.stdout.toString("utf8"))
		console.log(code)
		this.onProgress("log", "Map created");
		return true;
	}
}