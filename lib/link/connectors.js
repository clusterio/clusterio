...
class WebSocketBaseConnector extends events.EventEmitter {
	 ...
	 */
	send(type, data) {
		this._check(["handshake", "connected", "closing"]);
...
class WebSocketClientConnector extends WebSocketBaseConnector {
	...
	_attachSocketHandlers() {
		this._socket.on("close", (code, reason) => {
			...
			if (code === 4003) {
				this.emit("error", new errors.AuthenticationFailed("Authentication failed"));
				this._state = "closing";
			}
		...
		this._socket.on("unexpected-response", (req, res) => {
			...
			res.on('end', () => {
				...
				console.error(data);
				this._socket.emit("error", new Error(`Unexpected Response ${res.statusCode}`));
			});
		});
		*/
