...
class FactorioServer extends events.EventEmitter {
		...
		this._server.on('exit', (code, signal) => {
			if (this._state !== "stopping") {
				if (this._unexpected.length === 0) {
					this.emit('error', new Error("Factorio server unexpectedly shut down"));
	 ...
	 */
	async sendRcon(message) {
		this._check(["init", "running"])
		if (!this._rconReady) {
			await events.once(this, 'rcon-ready');
		}

		return await this._rconClient.send(message);
	}
