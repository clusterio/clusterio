...
async function listMods(config, instance) {
	return new Promise((resolve, reject) => {
		const instances = require("./../getInstances.js")(config).sync();
		if (instances.includes(instance.name)) {
			factorioAPI.init(false, instance.path("instanceMods"), instance.path("saves"), config.factorio_version);
